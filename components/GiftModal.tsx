...
import { Users, Coins, ChevronDown, Check, Star, Flame, Trophy, Gift as GiftIcon } from 'lucide-react';
...
const GiftModal: React.FC<GiftModalProps> = ({
  ...
  const renderIcon = (icon: string) => {
    ...
    if (isImage) {
      return <img src={icon} className="w-full h-full object-contain drop-shadow-md" alt="gift" />;
    }
    return <span className="text-3xl drop-shadow-md">{icon}</span>;
  };

  const tabs: { id: Gift['category']; label: string; icon: any }[] = [
    { id: 'popular', label: 'شائع', icon: Flame },
    { id: 'exclusive', label: 'حصري', icon: Star },
    { id: 'lucky', label: 'الحظ', icon: Trophy },
    { id: 'trend', label: 'ترند', icon: Star },
  ];
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-4 gap-3 scrollbar-hide">
           {filteredGifts.map(gift => (
              ...
              >
                 <div className="w-14 h-14 flex items-center justify-center group-hover:scale-110 transition-transform mb-1">
        ...
        <div className="p-4 bg-black/60 border-t border-white/5 flex items-center justify-between gap-4">
           <div className="flex flex-col">
              ...
              <div className="flex items-center gap-1">
                 {/* عرض الرصيد بشكل واضح مع ضمان تحويله لرقم */}
