...
const compressImage = (base64: string, maxWidth: number, maxHeight: number, quality: number = 0.1): Promise<string> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // تحويل إلى webp بجودة منخفضة جداً لتوفير مساحة في الـ Base64
    ...
    if (file) {
      ...
      reader.onload = async (event) => {
        if (event.target?.result) {
          ...
          // تصغير Avatar إلى 64x64، والغلاف إلى 300x120 بجودة منخفضة جداً
          const compressed = type === 'avatar' 
            ? await compressImage(raw, 64, 64, 0.15) 
            : await compressImage(raw, 300, 120, 0.1);
